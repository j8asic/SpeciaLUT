...
struct SomeStruct
{
    ...
    using table = TABULATE(SomeStruct::member_fn);
    void test_from_within()
    {

        SpeciaLUT::Chooser<table, 3> chooser;
...
using cuda_table = TABULATE(cuda_fn);
#endif

using table = TABULATE(host_fn);
using mem_table = TABULATE(SomeStruct::member_fn);
...
int main()
{
    ...
    SpeciaLUT::Chooser<table, 2, 2, 3, 2> test;
    ...
    SpeciaLUT::Chooser<mem_table, 3> member_chooser;
    ...
    SpeciaLUT::CudaChooser<cuda_table, 2, 2, 3, 2> cuda_test;
